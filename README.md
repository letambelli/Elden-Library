# Elden Library

Este site foi desenvolvido como parte do projeto da disciplina de Programação Front-end, sendo uma apicação simples que simula um site de uma livraria usando HTML, CSS e javascript.


## Objetivo

O objetivo desse projeto consiste em desenvolver um sistema para cadastro de entidades, que podem ser fornecedores, clientes, escolas, ongs e etc.

No caso da Elden Library, o site usa HTML, CSS e javascript para apresentar uma página inicial e simular o cadastro de um usuário ou entidade.


## Tecnologias Utilizadas

- HTML
- CSS
- JS


## Telas e Componentes

### Tela Inicial (index)

![Pagina Inicial](./imagens/img_readme/Captura%20de%20tela%202025-11-11%20133948.png)


### Sobre Nós

![Sobre Nós](./imagens/img_readme/Captura%20de%20tela%202025-11-11%20133957.png)


### Login

![Login](./imagens/img_readme/Captura%20de%20tela%202025-11-11%20134006.png)


### Cadastrar Como

![Cadastrar Como](./imagens/img_readme/Captura%20de%20tela%202025-11-11%20134029.png)


### Cadastrar Usuário

![Cadastrar Usuário](./imagens/img_readme/Captura%20de%20tela%202025-11-11%20134036.png)


### Cadastrar Entidade

![Cadastrar Entidade](./imagens/img_readme/Captura%20de%20tela%202025-11-11%20134117.png)
